...
# define MBED_CONF_APP_NTP_SERVER "80.86.38.193"
#else
...
static UbloxATCellularInterfaceN2xx *interface;
...
// Get NTP time
static void do_ntp(UbloxATCellularInterfaceN2xx *interface)
{
    UDPSocket sock;
...
static void use_connection(UbloxATCellularInterfaceN2xx *interface)
{
...
static void drop_connection(UbloxATCellularInterfaceN2xx *interface)
{
...
void test_udp_echo() {
    ...
    int sizes[] = { 1, 2, 5, 150, 160, 170, 180, 190, 200, 250 };
    ...
    for (x = 0; x < 10; x++) {
        do_udp_echo(&sock, &host_address, sizes[x]);
    }
...
void test_max_sockets() {
    ...
    interface->set_LocalListenPort(10000);
    ...
    for (int x = 1; x < (int) (sizeof (sock) / sizeof (sock[0])); x++) {
        interface->set_LocalListenPort(10000+x);
...
void test_connect_local_instance_last_test() {
    ...
    UbloxATCellularInterfaceN2xx *pLocalInterface = NULL;

    pLocalInterface = new UbloxATCellularInterfaceN2xx(MDMTXD, MDMRXD,
...
int main() {
    ...
    interface = new UbloxATCellularInterfaceN2xx(MDMTXD, MDMRXD,
                                    MBED_CONF_UBLOX_CELL_N2XX_BAUD_RATE,
                                    MBED_CONF_APP_DEBUG_ON);
